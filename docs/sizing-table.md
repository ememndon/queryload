...
the same local model in comparable tools, because QueryLoad adds no heavyweight
overhead of its own.
...
- **Disk:** budget roughly the size of your documents again for the encrypted
  index and vectors (embeddings + metadata), plus the model file (2 to 47 GB by
  tier). Example: 20 GB of documents is roughly 20 to 30 GB of index.
...
queue holds overflow. These are starting points, so measure with your own corpus.

| Users (concurrent) | Documents | Recommended server |
|--------------------|-----------|--------------------|
| 1 to 3 | up to ~50k | 32 GB RAM, 8+ cores; 14B model. GPU optional. |
| 4 to 10 | up to ~250k | 64 GB RAM, 12+ cores, 24 GB VRAM GPU; 24B to 27B model. |
| 10 to 25 | up to ~1M | 128 GB RAM, 16+ cores, 48 GB VRAM GPU; 70B model. |
